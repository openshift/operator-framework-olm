...
var _ = g.Describe("[sig-operator][Jira:OLM] OLMv0 within a namespace", func() {
	...
	g.It("PolarionID:27680-[OTP][Skipped:Disconnected]OLM Bundle support for Prometheus Types [Serial]", g.Label("NonHyperShiftHOST"), func() {
		...
		sub := olmv0util.SubscriptionDescription{
			...
			Channel:                "",
			IpApproval:             "Automatic",
			OperatorPackage:        "odf-prometheus-operator",
			SingleNamespace:        true,
			Template:               subTemplate,
		}
		exists, err := olmv0util.ClusterPackageExists(oc, sub)
		o.Expect(err).NotTo(o.HaveOccurred(), "failed to query packagemanifest for %s in %s", sub.OperatorPackage, sub.CatalogSourceNamespace)
		if !exists {
			g.Skip("SKIP:PackageMissing odf-prometheus-operator does not exist in catalog redhat-operators")
		}

		defaultChannel, channelErr := oc.AsAdmin().WithoutNamespace().Run("get").Args(
			"packagemanifest", sub.OperatorPackage, "-n", sub.CatalogSourceNamespace, "-o=jsonpath={.status.defaultChannel}",
		).Output()
		o.Expect(channelErr).NotTo(o.HaveOccurred(), "failed to get default channel for packagemanifest %s", sub.OperatorPackage)
		sub.Channel = strings.TrimSpace(defaultChannel)
		o.Expect(sub.Channel).NotTo(o.BeEmpty(), "default channel for packagemanifest %s is empty", sub.OperatorPackage)
	...
	g.It("PolarionID:33176-[OTP][Skipped:Disconnected]Enable generated operator component adoption for operators with single ns mode[Slow][Serial]", g.Label("NonHyperShiftHOST"), g.Label("original-name:[sig-operator][Jira:OLM] OLMv0 within a namespace PolarionID:33176-[Skipped:Disconnected]Enable generated operator component adoption for operators with single ns mode[Slow][Serial]"), func() {
		...
		olmv0util.NewCheck("expect", exutil.AsAdmin, exutil.WithoutNamespace, exutil.Contain, "InstallWaiting", exutil.Ok, []string{"operator.operators.coreos.com", subCockroachdb.OperatorPackage + "." + subCockroachdb.Namespace, "-o=jsonpath={.status.components.refs[?(.kind=='ClusterServiceVersion')].conditions[*].reason}"}).Check(oc)
