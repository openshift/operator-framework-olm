...
func createConfig(namespace string, clientCfg *restclient.Config) (*clientcmdapi.Config, error) {
	...
	credentials.ClientCertificate = clientCfg.CertFile
	if len(credentials.ClientCertificate) == 0 {
		credentials.ClientCertificateData = clientCfg.CertData
	}
	credentials.ClientKey = clientCfg.KeyFile
	if len(credentials.ClientKey) == 0 {
		credentials.ClientKeyData = clientCfg.KeyData
	}
