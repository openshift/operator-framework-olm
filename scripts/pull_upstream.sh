...
git fetch "${remote_name}" "${remote_ref}" || git fetch "${remote_name}"
...
git fetch "${remote_name}" +"${remote_ref}":refs/remotetags/"${remote_name}"/"${remote_ref}"
...
echo "Sync upstream ${remote_name} ${remote_ref}" > .git/COMMIT_MSG
echo "" >> .git/COMMIT_MSG 
git log --format=%B -n 1 --skip=1 "${temp_branch}" >> .git/COMMIT_MSG
