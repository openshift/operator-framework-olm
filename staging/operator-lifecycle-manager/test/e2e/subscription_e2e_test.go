...
func fetchSubscription(crc versioned.Interface, namespace, name string, checker subscriptionStateChecker) (*operatorsv1alpha1.Subscription, error) {
    ...
    err := wait.PollUntilContextTimeout(context.Background(), pollInterval, pollDuration, true, func(ctx context.Context) (bool, error) {
        var err error
        fetchedSubscription, err = crc.OperatorsV1alpha1().Subscriptions(namespace).Get(ctx, name, metav1.GetOptions{})
