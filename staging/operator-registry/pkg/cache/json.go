...
func (q *jsonBackend) metaDir(in metaKey) string {
	return filepath.Join(q.baseDir, jsonMetasDir, in.Schema, in.PackageName)
}
...
func (q *jsonBackend) SendMetas(_ context.Context, key metaKey, sender func([]byte) error) error {
	dir := q.metaDir(key)
